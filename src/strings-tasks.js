...
function isPalindrome(str) {
  const lowedStr = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
  return lowedStr.toLowerCase() === lowedStr.split('').reverse().join('');
}
...
function findLongestWord(sentence) {
  ...
  const longestWord = words.reduce((longest, currentWord) => {
    const cleanedWord = currentWord.replace(/[^a-zA-Z0-9]/g, '');
